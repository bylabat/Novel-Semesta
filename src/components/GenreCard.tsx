...
export function GenreCard({ genre, className }: GenreCardProps) {
  const Icon = (Icons as unknown as Record<string, LucideIcon>)[genre.icon] ?? Icons.BookOpen;
  ...
  return (
    ...
    >
      ...
      <div className="relative flex h-14 w-14 items-center justify-center rounded-2xl bg-secondary transition-all duration-300 group-hover:bg-primary/20 group-hover:scale-110">
        <Icon className="text-primary transition-colors" size={26} />
      </div>
      <div className="relative">
        <h3 className="font-display text-sm font-semibold text-foreground">{genre.name}</h3>
